...
class PomodoroTimer:
    ...
    def __init__(self):
        ...
        self.root.tk.call('wm', 'iconphoto', self.root._w, PhotoImage(file = 'tomatino.png'))

        self.s = ttk.Style()
        self.s.configure("TNotebook.Tab", font=("Ubuntu", 13))
        self.s.configure("TButton.Tab", font=("Ubuntu", 13))
        ...
        self.s = ttk.Style()
        self.s.configure('Custom.TButton', background='#F7D3D3', foreground='black', font=('Helvetica', 12))

        self.start_button = ttk.Button(self.grid_layout, text='Start', command=self.start_timer_thread, style='Custom.TButton')
        self.start_button.grid(row=0, column=0)

        self.skip_button = ttk.Button(self.grid_layout, text='Skip', command=self.skip_clock, style='Custom.TButton')
        self.skip_button.grid(row=0, column=1)

        self.reset_button = ttk.Button(self.grid_layout, text='Reset', command=self.reset_clock, style='Custom.TButton')
        self.reset_button.grid(row=0, column=2)


        self.pomodoro_counter_label = ttk.Label(self.grid_layout, text = 'Pomodoros: 0', font=("Ubuntu", 10), style='Custom.TButton')
